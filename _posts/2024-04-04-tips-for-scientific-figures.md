...
The "dream" of having a full final figure generated as a whole from a piece of code is just not worth it to the type of figures my papers usually have. Therefore only in very rare cases I even attempt to create a full figure or several panels directly from code. Since I have become very familiar with Inkscape, I can very quickly assemble plots into a figure with infinite customizability which would be very painful to do in code. Plus building the figures in a graphical interface really helps me 'play' with the pieces of data and make a visual narrative for the paper, which then I translate to text - I am a visual person and every story starts with images for me.

In concept, my apporoach is quite simple:
1. generate plots indivually as data is analyzed using [matplotlib](https://matplotlib.org/)/[seaborn](https://seaborn.pydata.org/);
2. assemble plots into a figure using [Inkscape](https://inkscape.org/);
3. uniformely process all figures into various formats (separate PNG, PDF, joint PDF).
